...
def random_change(result):
  for i in range(10):
    centroid_idx = randint(a=0,b=len(result['centers'])-1)
    col_idx = randint(a=0, b=len(result['centers'][centroid_idx])-1)
    result['centers'][centroid_idx][col_idx] = float(randint(a=0, b=int(max(result['centers'][centroid_idx]))))
...
print(bubbles.memdriver.__file__)
driver = bubbles.memdriver.MemDriver()
...
if __name__ == "__main__":
    ...
    while True:
        time.sleep(1)
        random_change(result)
        driver.put_result_id(result, result_id)
    try:
        server.join()
